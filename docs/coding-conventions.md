...
- `slf4j` 로그 메시지는 모두 한국어로 작성한다.
- 운영 대응에 필요한 식별자(`orderId`, `paymentId`, `eventId`, `reservationId`)는 로그 파라미터로 함께 남긴다.
- 로그 메시지는 비즈니스 상황이 드러나도록 작성하고, 불필요한 영어 문장이나 축약 표현을 사용하지 않는다.
